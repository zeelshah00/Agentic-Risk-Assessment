...
resource "google_compute_backend_service" "default" {
  count                 = var.enable_load_balancer ? 1 : 0
  name                  = "${var.app_name}-backend-prod"
  protocol              = "HTTP"
  port_name             = "http"
  timeout_sec           = 30
  enable_cdn            = false
...
resource "google_compute_url_map" "default" {
  count           = var.enable_load_balancer ? 1 : 0
  name            = "${var.app_name}-url-map-prod"
...
resource "google_compute_target_http_proxy" "default" {
  count   = var.enable_load_balancer ? 1 : 0
  name    = "${var.app_name}-http-proxy-prod"
...
resource "google_compute_target_https_proxy" "default" {
  count            = var.enable_load_balancer ? 1 : 0
  name             = "${var.app_name}-https-proxy-prod"
  url_map          = google_compute_url_map.default[0].id
...
resource "google_compute_managed_ssl_certificate" "default" {
  count = var.enable_load_balancer ? 1 : 0
  name  = "${var.app_name}-ssl-cert-prod"
...
resource "google_compute_global_forwarding_rule" "http" {
  count      = var.enable_load_balancer ? 1 : 0
  name       = "${var.app_name}-http-production"
...
resource "google_compute_global_forwarding_rule" "https" {
  count      = var.enable_load_balancer ? 1 : 0
  name       = "${var.app_name}-https-production"
